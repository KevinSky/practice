...
public class XA2Demo {
    ...
    public static void main(String[] args) {
        ...
        XADataSource xaDs1 = XA2Demo.getDataSource("jdbc:mysql://192.168.*.*:3306/mx2", "root", "root");
        XAConnection xaCon1 = null;
        XAResource xaRes1 = null;
        Connection conn1 = null;
        Statement stmt1 = null;

        XADataSource xaDs2 = XA2Demo.getDataSource("jdbc:mysql://192.168.*.*:3306/mx1", "root", "root");
        ...
        try {
            xaCon1 = XA2Demo.getXAConnetion(xaDs1);
            conn1 = XA2Demo.getConnection(xaCon1);
            stmt1 = conn1.createStatement();
            xaRes1 = xaCon1.getXAResource();

            xaCon2 = XA2Demo.getXAConnetion(xaDs2);
            conn2 = XA2Demo.getConnection(xaCon2);
            stmt2 = conn2.createStatement();
            xaRes2 = xaCon2.getXAResource();

            xaRes1.start(xid1, XAResource.TMNOFLAGS);
            stmt1.execute("INSERT INTO mx_user (uid,name,add_time) VALUES (148,'aaabbb','"
                    + dateFormat.format(new Date()) + "')");
            xaRes1.end(xid1, XAResource.TMSUCCESS);

            if (xaRes2.isSameRM(xaRes1)) {
                xaRes2.start(xid1, XAResource.TMNOFLAGS);
                stmt2.execute("INSERT INTO mx_user (uid,name,add_time) VALUES (148,'aaabbb','"
                        + dateFormat.format(new Date()) + "')");
